...
const publishingTags = Object.entries({
  'App-Name': 'AR-IO-HYPER-ANT-Deploy',
  'App-Version': '0.0.1',
  'AR-IO-HYPER-ANT-Version': `${version}`,
...
console.log('Generated HYPER-ANT source code data item with id: ' + data.id);

if (!dryRun) {
  console.log('Publishing HYPER-ANT Source code to Arweave...');
  await turbo.uploadSignedDataItem({
    dataItemSizeFactory: () => data.getRaw().byteLength,
    dataItemStreamFactory: () => data.getRaw(),
  });
}
console.log('Tagged HYPER-ANT source code tx id: ' + data.id);
